...
// ✅ BLOCO DE CONTADORES (fora do botão toggle)
const campos = [
  { id: "nome", max: 100 },
  { id: "email", max: 254 },
  { id: "senha", max: 64 },
  { id: "confirmarSenha", max: 64 }
];

campos.forEach(campo => {
  const input = document.getElementById(campo.id);
  const count = document.getElementById(`${campo.id}-count`);
  if (input && count) {
    input.addEventListener("input", () => {
      count.textContent = `${input.value.length}/${campo.max}`;
    });
  }
});